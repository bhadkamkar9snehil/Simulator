...
2. establish CI and release hygiene;
...
The following is a source-review baseline using conventional McCabe/Radon-style reasoning. Exact values should be generated by CI once automated complexity tooling is added.
...
Dependency removal MUST be verified through CI/import scans and tests rather than assumed.
...
- Radon/Xenon or equivalent complexity reporting/gating;
...
## 20. Proposed Complexity Gate

Once automated metrics are available:
...
A CI baseline should report both:

- absolute current complexity;
- complexity introduced/changed by the pull request.
...
- exact CC metrics are automatically visible in CI;
